...
API_KEY_FILEPATH: Path = Path.home() / ".config" / "openai" / "api-key.txt"
API_KEY = API_KEY_FILEPATH.read_text().strip()
...
if USE_PROXY:
    ...
    proxies = {
        #          [协议]://  [地址]  :[端口]
        "http": os.getenv("http_proxy"),  # 再例如  "http":  "http://127.0.0.1:7890",
        "https": os.getenv("https_proxy"),  # 再例如  "https": "http://127.0.0.1:7890",
    }
else:
    proxies = None
