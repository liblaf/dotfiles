...
class Module:
    ...
    def _apply_root_file(self, ctx: Context, source: Path, relative: Path) -> bool:
        ...
        parts[1:] = [f"exact_{part}" for part in parts[1:]]
...
def main() -> None:
    ...
    profile: Profile = Profile.load(args.profile)
    ctx: Context = Context(output_dir=Path("home"))
    profile.apply(ctx)
